...
async function startMigrationToFirebase(onProgressUpdate) {
  ...
  try {
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      // Keep leading zeros if any, or normalize
 ...
 if (onProgressUpdate) onProgressUpdate(✅ បាន Migrate បុគ្គលិកចំនួន នាក់រួចរាល់!);
 } catch (err) {
 ...
 await batch.commit();
 if (onProgressUpdate) onProgressUpdate(✅ បាន Migrate ឯកសារ In/Out ចំនួន ឯកសាររួចរាល់!);
 } catch (err) {
 ...
 await batch.commit();
 if (onProgressUpdate) onProgressUpdate(✅ បាន Migrate ព័ត៌មាន និងសកម្មភាពចំនួន អត្ថបទរួចរាល់!);
 } catch (err) {
 ...
 await batch.commit();
 if (onProgressUpdate) onProgressUpdate(✅ បាន Migrate QAC Checklist ចំនួន ចំណុចរួចរាល់!);
 } catch (err) {
