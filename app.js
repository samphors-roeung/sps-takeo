...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize Native Staff Directory
  if (window.initStaffDirectory) window.initStaffDirectory();
...
// ==================== NATIVE CLOUD FIRESTORE STAFF DIRECTORY ====================
let allStaffMembersList = [];
let activeStaffFilterDept = 'all';
let currentModalStaff = null;
let currentModalTab = 'part1';
...
window.initStaffDirectory = function() {
  if (window.StaffService && window.StaffService.subscribe) {
    window.StaffService.subscribe((list) => {
      if (list && list.length > 0) {
        allStaffMembersList = list;
        renderStaffCardsGrid();
        const badge = document.getElementById('staff-total-badge');
        if (badge) badge.innerText = list.length;
        const statCount = document.getElementById('staff-count');
        if (statCount) statCount.innerText = list.length;
      }
    });
  }
};

function renderStaffCardsGrid() {
  const container = document.getElementById('staff-cards-grid');
  if (!container) return;

  const searchKeyword = (document.getElementById('staff-search-input')?.value || '').toLowerCase().trim();

  const filtered = allStaffMembersList.filter(s => {
    // Dept filter
    if (activeStaffFilterDept !== 'all') {
      const d = (s.department || '').toLowerCase();
      if (activeStaffFilterDept === 'Management' && !d.includes('manage') && !d.includes('office')) return false;
      if (activeStaffFilterDept === 'GEP' && !d.includes('gep')) return false;
      if (activeStaffFilterDept === 'Highschool' && !d.includes('high')) return false;
      if (activeStaffFilterDept === 'Primary' && !d.includes('prim')) return false;
      if (activeStaffFilterDept === 'Operation' && !d.includes('op') && !d.includes('secu') && !d.includes('clean')) return false;
    }
    // Search keyword
    if (searchKeyword) {
      ...
      return name.includes(searchKeyword) || id.includes(searchKeyword) || role.includes(searchKeyword);
    }
    return true;
  });
  ...
  if (filtered.length === 0) {
    container.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #94a3b8;">
      ...
      <div>មិនមានបុគ្គលិកត្រូវនឹងពាក្យស្វែងរកនេះទេ</div>
    </div>`;
    return;
  }
  ...
  container.innerHTML = filtered.map(s => {
    ...
    ['part1', 'part2', 'part3'].forEach(pk => {
      ...
      if (p) {
        Object.values(p).forEach(item => {
          if (item && (item.checked === true || item.fileUrl || item.fileName)) done++;
        });
      }
    });
    ...
    return `
      <div class="staff-card" onclick="openStaffDetailModal('${s.id}')" style="background: white; border-radius: 16px; padding: 18px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); border: 1px solid #e2e8f0; cursor: pointer; transition: all 0.2s ease; display: flex; flex-direction: column; justify-content: space-between;">
        <div style="display: flex; align-items: center; gap: 14px; margin-bottom: 12px;">
          <div style="width: 56px; height: 56px; border-radius: 50%; overflow: hidden; border: 2px solid #e0f2fe; flex-shrink: 0; background: #f1f5f9;">
            <img src="${photoSrc}" alt="${s.name}" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://lh3.googleusercontent.com/d/1PoR7-o5Ea4QstFQ2QLcw0WHuV6dKA480'">
          </div>
          <div style="flex: 1; min-width: 0;">
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 6px;">
              <span style="font-size: 0.76rem; background: #e0f2fe; color: #0071ba; font-weight: 700; padding: 2px 8px; border-radius: 8px;">ID: ${s.id}</span>
              <span style="font-size: 0.72rem; font-weight: 700; color: ${badgeColor}; display: flex; align-items: center; gap: 4px;">
                ${isComplete ? '<i class="fa-solid fa-circle-check"></i> ពេញលេញ' : (done > 0 ? `${done}/${total} ឯកសារ` : '<i class="fa-solid fa-circle-xmark"></i> មិនទាន់មាន')}
              </span>
            </div>
            <h4 style="margin: 4px 0 2px; font-size: 0.98rem; color: #0f172a; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${s.name}</h4>
            <div style="font-size: 0.8rem; color: #64748b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${s.role || 'Staff Member'}</div>
          </div>
        </div>

        <div style="background: #f8fafc; border-radius: 10px; padding: 8px 12px; font-size: 0.78rem; color: #475569; display: flex; justify-content: space-between; align-items: center;">
          <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 160px;"><i class="fa-solid fa-building-user" style="color: #94a3b8; margin-right: 4px;"></i>${s.department || 'Takeo Campus'}</span>
          <span style="color: #0071ba; font-weight: 600;"><i class="fa-solid fa-arrow-right"></i> មើលឯកសារ</span>
        </div>
      </div>
    `;
  }).join('');
...
window.filterStaffDirectory = function() {
  renderStaffCardsGrid();
};

window.filterStaffDept = function(dept, btn) {
  activeStaffFilterDept = dept;
  const btns = document.querySelectorAll('#staff-dept-filters .cat-filter-btn');
...
window.toggleStaffViewMode = function() {
  ...
  const controls = document.getElementById('staff-native-controls');
  const iframeBox = document.getElementById('staff-legacy-iframe-container');
  const btn = document.getElementById('btn-toggle-staff-mode');

  if (!cardsGrid || !iframeBox) return;

  const isCardsVisible = cardsGrid.style.display !== 'none';
  if (isCardsVisible) {
    cardsGrid.style.display = 'none';
    if (controls) controls.style.display = 'none';
    iframeBox.style.display = 'block';
    if (btn) btn.innerHTML = '<i class="fa-solid fa-grip"></i> ប្តូរទៅ Firebase Cloud (Realtime)';
  } else {
    cardsGrid.style.display = 'grid';
    if (controls) controls.style.display = 'flex';
    iframeBox.style.display = 'none';
    if (btn) btn.innerHTML = '<i class="fa-solid fa-table"></i> ប្តូរទៅ Google Sheet WebApp';
  }
};

window.openStaffDetailModal = function(staffId) {
  const cleanId = String(staffId).trim().replace(/^0+/, '');
  const staff = allStaffMembersList.find(s => {
  ...
  // Set Profile Header
  const photo = document.getElementById('modal-staff-photo');
  if (photo) photo.src = staff.photoUrl || staff.photo || 'https://lh3.googleusercontent.com/d/1PoR7-o5Ea4QstFQ2QLcw0WHuV6dKA480';
  document.getElementById('modal-staff-name').innerText = staff.name || 'Staff Member';
  document.getElementById('modal-staff-role').innerText = staff.role || 'Position';
  document.getElementById('modal-staff-id').innerText = `ID: ${staff.id}`;
  document.getElementById('modal-staff-dept').innerText = staff.department || 'Department';
  document.getElementById('modal-staff-phone').innerHTML = `<i class="fa-solid fa-phone"></i> ${staff.phone || 'N/A'}`;

  // Switch to Part 1
  switchStaffModalTab('part1');
...
window.switchStaffModalTab = function(tabKey) {
  ...
  // Section Title
  const titleEl = document.getElementById('staff-checklist-section-title');
  if (titleEl) {
    if (tabKey === 'part1') titleEl.innerText = 'Staff Personal Information Checklist';
    if (tabKey === 'part2') titleEl.innerText = 'Recruitment & Pre-Employment Records';
    if (tabKey === 'part3') titleEl.innerText = 'Ongoing Employment Updates';
  }

  // Render Items
  const container = document.getElementById('staff-checklist-items-container');
  if (!container || !currentModalStaff) return;

  const labels = STAFF_CHECKLIST_LABELS[tabKey] || [];
  const partData = currentModalStaff[tabKey] || {};
  ...
  container.innerHTML = labels.map((label, idx) => {
    ...
    return `
      <div style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: ${isChecked ? '#f0fdf4' : '#fff'}; border: 1px solid ${isChecked ? '#bbf7d0' : '#f1f5f9'}; border-radius: 10px;">
        <div style="display: flex; align-items: center; gap: 12px;">
          <span style="font-size: 1.1rem; color: ${isChecked ? '#10b981' : '#ef4444'};">
            <i class="fa-solid ${isChecked ? 'fa-circle-check' : 'fa-circle-xmark'}"></i>
          </span>
          <span style="font-size: 0.92rem; color: ${isChecked ? '#166534' : '#334155'}; font-weight: ${isChecked ? '600' : '400'};">
            ${label}
          </span>
        </div>
        ${fileUrl ? `
          <a href="${fileUrl}" target="_blank" style="font-size: 0.78rem; background: #0071ba; color: white; padding: 4px 10px; border-radius: 6px; text-decoration: none; display: flex; align-items: center; gap: 4px;">
